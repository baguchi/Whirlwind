package baguchan.whirl_wind.util;
...
public class JigjawHelper {
    ...
    public static void registerJigsaw(MinecraftServer server, ResourceLocation poolLocation, ResourceLocation nbtLocation, int weight) {
        ...
        Optional<Holder.Reference<StructureProcessorList>> processorListHolder = processorListRegistry.get(EMPTY_PROCESSOR_LIST_KEY);
        if (processorListHolder.isEmpty()) return;

        StructurePoolElement element = SinglePoolElement.legacy(nbtLocation.toString(), processorListHolder.get()).apply(StructureTemplatePool.Projection.RIGID);
